...
void entry(unsigned long magic, unsigned long addr) {
    ...
    boot_blk_t *boot_blk_ptr;//boot block pointer
    /* Clear the screen. */
    clear();
    ...
    if (CHECK_FLAG(mbi->flags, 3)) {
        ...
        while (mod_count < mbi->mods_count) {
            ...
            boot_blk_ptr=(boot_blk_t*)mod->mod_start;//get the address of boot block pointer
    ...
    /* Init the PIC */
    i8259_init();
    init_filesys(boot_blk_ptr);
    paging_init();//initialize paging
    
    /* Initialize devices, memory, filesystem, enable device interrupts on the PIC, any other initialization stuff... */
